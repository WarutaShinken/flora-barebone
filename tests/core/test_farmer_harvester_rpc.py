...
async def test_farmer_reward_target_endpoints(bt, harvester_farmer_environment):
    ...
    assert config["farmer"]["xfl_target_address"] == encode_puzzle_hash(observer_farmer, "xch")
    assert config["pool"]["xfl_target_address"] == encode_puzzle_hash(observer_pool, "xch")
